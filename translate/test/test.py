...
TRAIN_FILE = 'D:/python/translate/nmt/train.txt'
DEV_FILE = "D:/python/translate/nmt/dev.txt"  # 验证集
SAVE_FILE = 'D:/python/translate/save/model.pt'         # 模型保存路径
...
def interactive_translate(model, data):
    while True:
        # 用户输入英文句子
        input_sentence = input("请输入要翻译的英文句子（输入 'exit' 退出）: ")
        ...
        # 将输入的英文句子转换为模型输入格式
        input_tokens = ["BOS"] + word_tokenize(input_sentence.lower()) + ["EOS"]
        input_ids = [data.en_word_dict.get(word, UNK) for word in input_tokens]

        # 将输入的英文句子转为张量并放入 DEVICE
        src = torch.tensor(input_ids).long().to(DEVICE)
        src = src.unsqueeze(0)
...
# 开始交互式翻译
interactive_translate(model, data)
